"""Axiom-X Invariant Contracts — mathematical/physical contracts for invariant execution layer.

STATUS: **design** — contracts define the mathematical/physical substrate;
conforming implementations (Python reference, GPU/HIP/CUDA/OpenCL) must satisfy these contracts.

The invariant contract system is substrate-independent:
- Python/reference = mathematical contract (ground truth)
- GPU/HIP/CUDA/OpenCL = conforming implementations
- Each operation declares: INPUT CONTRACT → OPERATION → OUTPUT CONTRACT
- Invariant validation results feed into convergence verifier D0-D4
- Provenance is attached to render receipts
"""
...
class InvariantContract:
    """Mathematical/physical contract for an operation.

    Each field is an optional sub-contract; if None, that class of invariant
    is not checked for this operation.
    """
    energy: Optional["EnergyContract"] = field(default_factory=lambda: EnergyContract())
    geometry: Optional["GeometryContract"] = field(default_factory=lambda: GeometryContract())
    radiometry: Optional["RadiometryContract"] = field(default_factory=lambda: RadiometryContract())
    probability: Optional["ProbabilityContract"] = field(default_factory=lambda: ProbabilityContract())
    topology: Optional["TopologyContract"] = field(default_factory=lambda: TopologyContract())
    numerical: Optional["NumericalContract"] = field(default_factory=lambda: NumericalContract())
    temporal: Optional["TemporalContract"] = field(default_factory=lambda: TemporalContract())
...
class GeometryContract:
    """Geometric invariants contract.

    Invariants: rotation matrix orthogonality, distance preservation,
    normal orientation consistency.
    """
    rotation_matrix_valid: bool = True
    distance_preservation: bool = True
    normal_orientation: bool = True
    absolute_tolerance: float = 1e-8


@dataclass
class RadiometryContract:
    """Radiometry invariants contract.

    Invariants: non-negative radiance, BRDF reciprocity.
    """
...
class ProbabilityContract:
    """Probability / Monte Carlo invariants contract.

    Invariants: unbiased estimation, seed determinism, variance bounds.
    """
    unbiased: bool = True
    seed_determinism: bool = True
    variance_within: Optional[float] = None  # e.g., 0.05 for 5% tolerance
    absolute_tolerance: float = 1e-8


@dataclass
class TopologyContract:
    """Topological invariants contract.

    Invariants: mesh connectivity, manifold integrity, vertex referencing.
    """
    mesh_connectivity: bool = True
    manifold_integrity: bool = True
    vertex_referencing: bool = True
    absolute_tolerance: float = 1e-8


@dataclass
class NumericalContract:
    """Numerical invariants contract.

    Invariants: no NaN/Inf, precision contract, catastrophic cancellation guard.
    """
    fp_stability: bool = True  # no NaN/Inf
    precision_contract: str = "fp32"  # "fp32", "fp64", "mixed"
    catastrophic_cancellation: bool = True
    absolute_tolerance: float = 1e-8


@dataclass
class TemporalContract:
    """Temporal invariants contract.

    Invariants: deterministic seed, energy conservation accumulated,
    frame-to-frame consistency.
    """
    deterministic_seed: bool = True
    energy_conservation_accumulated: bool = True
    frame_to_frame_consistency: float = 1e-6  # max delta between frames
    absolute_tolerance: float = 1e-8


@dataclass
class InvariantValidation:
    """Complete validation result for one operation.

    Fields are given defaults so they can be created with keyword arguments.
    """
    operation_id: str = ""
    invariant_name: str = ""
...
class ProvenanceRecord:
    """Provenance record for invariant validation."""
    render_id: str = ""
...
def validate_energy(in_L: float, out_L: float, contract: EnergyContract) -> InvariantValidation:
    ...
    if contract.conserved:
        ...
        if out_L > in_L + contract.absolute_tolerance:
            ...
            failures.append(
                "Energy not conserved: L_out=%f > L_in=%f + %f"
                % (out_L, in_L, contract.absolute_tolerance)
            )
    ...
    if passed and contract.relative_tolerance > 0:
        ...
        if rel_error > contract.relative_tolerance:
            ...
            failures.append(
                "Relative energy error %.6f > %.6f" % (rel_error, contract.relative_tolerance)
            )
            details_parts.append("relative_error=%f" % rel_error)

    metric_value = max(metric_value, 0.0)
    return InvariantValidation(
        "energy",  # invariant_name
        passed,  # overall_passed
        metric_value,  # metric_value
        contract.absolute_tolerance,  # threshold
        "energy conserved within tolerance",  # details
    )
...
def validate_geometry_rotation(R: np.ndarray, contract: GeometryContract) -> InvariantValidation:
    ...
    metric_value = max(metric_value, 0.0)
    return InvariantValidation(
        "geometry_rotation",  # invariant_name
        passed,  # overall_passed
        metric_value,  # metric_value
        contract.absolute_tolerance,  # threshold
        "rotation matrix valid" if passed else "rotation matrix invalid",  # details
    )
...
def validate_geometry_distance_preservation(
    ...
    metric_value = max(metric_value, 0.0)
    return InvariantValidation(
        "geometry_distance_preservation",  # invariant_name
        passed,  # overall_passed
        metric_value,  # metric_value
        contract.absolute_tolerance,  # threshold
        "distance preserved" if passed else "distance not preserved",  # details
    )
...
def validate_radiometry_non_negative(radiance: float, contract: RadiometryContract) -> InvariantValidation:
    ...
    metric_value = max(metric_value, 0.0)
    return InvariantValidation(
        "radiance_non_negative",  # invariant_name
        passed,  # overall_passed
        metric_value,  # metric_value
        contract.absolute_tolerance,  # threshold
        "radiance non-negative" if passed else "radiance %f negative" % metric_value,  # details
    )
...
def validate_brdf_reciprocity(brdf_func, wi: np.ndarray, wo: np.ndarray, contract: RadiometryContract) -> InvariantValidation:
    ...
    try:
        ...
        return InvariantValidation(
            "brdf_reciprocity",  # invariant_name
            False,  # overall_passed
            metric_value,  # metric_value
            contract.absolute_tolerance,  # threshold
            "brdf evaluation failed: %s" % str(e)[:60],  # details
        )
    ...
    return InvariantValidation(
        ...
        metric_value,  # metric_value
        contract.absolute_tolerance,  # threshold
        "; ".join(details_parts) if details_parts else "BRDF reciprocity holds",  # details
    )
...
def validate_unbiased_estimator(samples: np.ndarray, ground_truth: float, contract: ProbabilityContract) -> InvariantValidation:
    ...
    if len(samples) == 0:
        ...
        return InvariantValidation(
            "unbiased_estimator",  # invariant_name
            False,  # overall_passed
            metric_value,  # metric_value
            contract.absolute_tolerance,  # threshold
            "no samples provided",  # details
        )
    ...
    return InvariantValidation(
        ...
        metric_value,  # metric_value
        contract.absolute_tolerance,  # threshold
        "estimate=%f, ground_truth=%f, error=%f" % (estimate, ground_truth, error),  # details
    )
...
def validate_seed_determinism(render_result1, render_result2, contract: ProbabilityContract) -> InvariantValidation:
    ...
    metric_value = max(metric_value, 0.0)
    return InvariantValidation(
        "seed_determinism",  # invariant_name
        passed,  # overall_passed
        metric_value,  # metric_value
        0.0,  # threshold (deterministic means exactly equal)
        "outputs identical" if passed else "outputs differ between same-seed runs",  # details
    )
...
def validate_mesh_connectivity(vertices: np.ndarray, indices: np.ndarray, contract: TopologyContract) -> InvariantValidation:
    ...
    metric_value = max(metric_value, 0.0)
    return InvariantValidation(
        "mesh_connectivity",  # invariant_name
        passed,  # overall_passed
        metric_value,  # metric_value
        contract.absolute_tolerance,  # threshold
        "; ".join(details_parts) if details_parts else "mesh connectivity valid",  # details
    )
...
def validate_fp_stability(values: np.ndarray, contract: NumericalContract) -> InvariantValidation:
    ...
    if isinstance(metric_value, float) and (np.isnan(metric_value) or np.isinf(metric_value)):
        metric_value = 1.0  # mark as failed

    return InvariantValidation(
        "fp_stability",  # invariant_name
        passed,  # overall_passed
        metric_value,  # metric_value
        contract.absolute_tolerance,  # threshold
        "floating-point stable" if passed else "numerical instability detected",  # details
    )
...
def validate_temporal_determinism(render_result1, render_result2, contract: TemporalContract) -> InvariantValidation:
    ...
    metric_value = max(metric_value, 0.0)
    return InvariantValidation(
        "temporal_determinism",  # invariant_name
        passed,  # overall_passed
        metric_value,  # metric_value
        contract.frame_to_frame_consistency,  # threshold
        "frames identical" if passed else "frames differ between consecutive renders",  # details
    )
...
def validate_energy_accumulated(accumulated: float, contract: TemporalContract) -> InvariantValidation:
    ...
    metric_value = max(metric_value, 0.0)
    return InvariantValidation(
        "energy_accumulated",  # invariant_name
        passed,  # overall_passed
        metric_value,  # metric_value
        contract.absolute_tolerance,  # threshold
        "accumulated energy non-negative" if passed else "accumulated energy %f negative" % metric_value,  # details
    )
...
class InvariantKernel:
    ...
    def validate_output(
        ...
        if self.contract is not None and hasattr(self.contract, 'energy'):
            ...
            results["energy"] = {
                ...
                "metric_value": energy_val.metric_value,
                "threshold": energy_val.threshold,
                "details": energy_val.details,
            }
            if not energy_val.overall_passed:
                ...
                metrics["energy_error"] = energy_val.metric_value
    ...
    def attach_provenance(self, receipt: Dict[str, Any], validation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.contract is not None:
            receipt["invariant_contract_checked"] = [
                k for k in self.contract.keys()
            ]
