...
name: mrs-crew
description: >-
  Orchestrates the local MRS five-role crew (architect, builder, implementor,
  reviewer, inspector). Use when the user asks to use the crew, subagents,
  or to design→build→implement→review→inspect a feature instead of doing all
  roles in one pass.
---
...
`docs/governance/cecp/trails/<trail-id>/` exists with the five stage files (+
...
Read-only roles (Architect, Reviewer, Inspector): prefer explore / insist no file writes.  
