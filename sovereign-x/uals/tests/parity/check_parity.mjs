...
import { renderAxiomX } from "../../../../mrs/packages/renderer-core/src/render/rt4d/print/AxiomXSampler.js";

const [wArg, hArg, sppArg, seedArg, binFile] = process.argv.slice(2);
if (!wArg || !hArg || !sppArg || !seedArg || !binFile) {
  console.error("usage: node check_parity.mjs <w> <h> <spp> <seed> <gpu.bin>");
  process.exit(2);
}
...
const ref = renderAxiomX(seed, spp, width, height);
...
console.log(`cpu.rt4d.print reference vs OpenCL Axiom X (${width}x${height} spp=${spp} seed=${seed})`);
